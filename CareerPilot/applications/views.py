...
def dashboard(request):
	...
	if query:
		applications = applications.filter(company__icontains=query) | applications.filter(job_title__icontains=query)
	if status:
		applications = applications.filter(status=status)
	paginator = Paginator(applications, 8)
	page = paginator.get_page(request.GET.get('page'))
	return render(request, 'dashboard.html', {'applications': page, 'query': query, 'status': status, 'status_choices': JobApplication.Status.choices})
